...
class Node10 implements Comparable<Node10>{
	...
	public Node10(int index, int distance) {
		super();
	...
	@Override
	public int compareTo(Node10 other) {
...
public class main1505Reverse {
	...
	public static ArrayList<ArrayList<Node10>> graph = new ArrayList<ArrayList<Node10>>();
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		d3 = new int[N+1];
		for(int i=0;i<=N;i++) {
			graph.add(new ArrayList<Node10>());
		}
		for(int i=0;i<E;i++) {
			...
			graph.get(a).add(new Node10(b,c));
			graph.get(b).add(new Node10(a,c));	//양방향 그래프이므로 반대로도 이어줌
		}
		Arrays.fill(d1, INF);
		Arrays.fill(d2, INF);
		Arrays.fill(d3, INF);
		...
		int v1Toend = d3[v1];	//v1 -> end 최소거리
		int v2Toend = d3[v2];	//v2 -> end 최소거리
	...
	private static void dijkstra(int start, int[] d) {
		PriorityQueue<Node10> pq = new PriorityQueue<Node10>();
		boolean[] visit = new boolean[N+1];
		d[start] = 0;
		pq.offer(new Node10(start,0));
		while(!pq.isEmpty()) {
			Node10 node = pq.poll();
			...
			if(visit[now] == false) {
				...
				for(int i=0;i<graph.get(now).size();i++) {
					...
					if(d[graph.get(now).get(i).getIndex()] > cost) {
						...
						pq.offer(new Node10(graph.get(now).get(i).getIndex(),cost)); 
					}
				}
			}
		}
